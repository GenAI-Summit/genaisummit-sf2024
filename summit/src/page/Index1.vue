<template>
 ...
 <div class="video_container" @wheel="handleWheel">
  <div class="video_box" :style="video_boxStyle">
    <img id="video" src="../assets/images/ico_background.png" alt="" srcset="">
    <!-- <video id="video" src="../assets/images/video.mp4" autoplay loop muted></video> -->
  </div>
</div>
...
export default {
  ...
  setup() {
    ...
    const video_boxStyle = ref({
      width: '75%',
      height: '50%'
    })
    const speed = 0.5
    const BSStatus = ref(false)
    const handleWheel = (event) => {
      var deltaY = event.deltaY;
      var containerTop = document.querySelector('.video_container').getBoundingClientRect().top;
      // 如果.container元素头部到浏览器头部的距离小于等于0，则执行放大视频和显示内容的操作
        if (deltaY < 0) {
          if(containerTop > 80) return
          if(video_boxStyle.value.width == '75%'){
            BSStatus.value = true
            document.body.style.overflow = 'auto';

            return
          }
          BSStatus.value = false
          video_boxStyle.value.width =  Number(video_boxStyle.value.width.replace('%','')) - Number(speed) + '%'
          video_boxStyle.value.height =  Number(video_boxStyle.value.height.replace('%','')) - 2*Number(speed) + '%'

          
        } else {
          if(video_boxStyle.value.width == '100%'){
            BSStatus.value = true
            document.body.style.overflow = 'auto';
            return
          }
    ...
    onMounted(() => {
      ...
      window.addEventListener('scroll', function() {
        ...
        if(Math.floor(containerTop) == 80 && !BSStatus.value){
          document.body.style.overflow = 'hidden';
        }else{
          document.body.style.overflow = 'auto';
        }
        ...
        // if (containerTop <= 0) {
        //   var scale = (scrollPosition + Math.abs(containerTop)) / window.innerHeight;
